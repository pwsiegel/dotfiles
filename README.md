...
| iTerm color scheme | iTerm prefs | Import `iterm/*.itermcolors` (`open iterm/*.itermcolors` registers them as Color Presets), then `Preferences → Profiles → Colors → Color Presets...` → pick `Solarized Dark Blue fg`. Pairs with the nvim solarized8 theme. |
